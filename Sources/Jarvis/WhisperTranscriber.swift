...
struct WhisperTranscriber {
    private var executablePath: String? {
        UserDefaults.standard.string(forKey: Settings.whisperExecutableKey)
    }

    private var modelPath: String? {
        UserDefaults.standard.string(forKey: Settings.modelPathKey)
    }
    ...
    func transcribe(_ audioURL: URL, dictionary: [DictionaryEntry]) async throws -> String {
        ...
        let outputPrefix = FileManager.default.temporaryDirectory
            .appendingPathComponent("jarvis-output-\(UUID().uuidString)")
